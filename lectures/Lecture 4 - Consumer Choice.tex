...
% ------------------------------------------------------------------------------------------------
\begin{frame}{The \navy{Optimality Condition}}
  We have just found our first \bgNavy{optimality condition}! Now that we have derived this from first principles, we don't have to do that again!
  ...
  \pause\bigskip
  \begin{itemize}
    \item The \navy{optimality condition} tells us that the consumer maximizes utility when the marginal utility per dollar spent on both goods is equal.
...
\begin{frame}{The Optimality Condition}
  We can also rearrange the \navy{optimality condition} as follows:
...
% ------------------------------------------------------------------------------------------------
\begin{frame}{The Optimality Condition}
  So far, we only considered the case where baskets contain two goods. However, the \navy{optimality condition} holds for any number of goods.
  ...
  \pause\bigskip
  \begin{center}
    This is a \emph {key insight}! This \navy{optimality condition} is how we think individuals balance \navy{trade-offs} on what to buy
  \end{center}
\end{frame}
% ------------------------------------------------------------------------------------------------
...
\begin{frame}{Finding the Optimal Bundle}
  Now that we know something about the optimal bundle (from the \navy{optimality condition}), how do we actually find it?

  \begin{itemize}
    \item Well we also know that the consumer has only  \$$I$ to spend.

    \item This gives us a system of two equations (the \navy{optimality condition} and the budget constraint), and two unknowns (quantity consumed of goods $x$ and $y$).
...
\begin{frame}{Finding the Optimal Bundle}
  ...
  \only<1>{
    \bigskip
    Calculate $MU_x$ and $MU_y$ and the \navy{optimality condition}
  }

  \only<2->{
    \bigskip
    $MU_x = \frac{y^{\frac{1}{2}}}{2x^{\frac{1}{2}}}$ and $MU_y = \frac{x^{\frac{1}{2}}}{2y^{\frac{1}{2}}}$
    
    \bigskip
    Then our \navy{optimality condition} is $\frac{y^{\frac{1}{2}}}{2x^{\frac{1}{2}}} / \frac{x^{\frac{1}{2}}}{2y^{\frac{1}{2}}} = \frac{2}{3} $
    ...
    \pause\bigskip
    Plugging the \navy{optimality condition} into our budget constraint
    $$
      2x + 3y = 24 \implies 2x + 3(\frac{2}{3} x) = 24 \implies x^* = 6
    $$

    \pause
    Plugging $x^*$ back into the \navy{optimality condition}, $y^* = \frac{2}{3} 6 = 4$
  }
\end{frame}
% ------------------------------------------------------------------------------------------------
...
\begin{frame}{Finding the Optimal Bundle}
  ...
  \bigskip
  Take the rearranged \navy{optimality condition} $y = x\frac{P_x}{P_y}$. Plug into budget constraint
  $$
...
\begin{frame}{Cobb-Douglas}
  $U(x,y) = x^\alpha y^\beta$. Our \navy{optimality condition} is 
...
% ------------------------------------------------------------------------------------------------
\begin{frame}{Corner Solutions}
  At a corner solution only one good is consumed at a positive quantity, e.g. $x^{*}>0$ \& $y^{*}=0$
  
  \bigskip
  Occurs when preferences are not well-behaved, such as by crossing one of the axis
  
  \bigskip
  Optimal solution is to spend all income on one good and none on the other good  
\end{frame}
% ------------------------------------------------------------------------------------------------
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        % Indifference curve for corner solution
        \addplot[domain = 0:50, restrict y to domain = 0:25, samples = 400, color = alice!40!white, very thick]{-1.2*x + 58};
        % \node [right,color=alice!25!white] at (axis cs: 12, 4/12) {$U = 2$};
        
        \addplot[domain = 0:50, restrict y to domain = 0:25, samples = 400, color = alice!70!white, very thick]{-1.2*x + 48};

        \addplot[domain = 0:50, restrict y to domain = 0:25, samples = 400, color = alice!100!white, very thick]{-1.2*x + 38};

        % Highlight Point (corner solution)
        \addplot[color = black, mark = *, only marks, mark size = 2pt] 
          coordinates {(40, 0)};
        \node [anchor = south west] at (axis cs:40, 0) {$A$};
...
\begin{frame}{Perfect Complements}
  ...
  \pause\bigskip
  $ax = by$ is therefore our \navy{optimality condition}! Our budget constraint is 
  $$
    I = P_x x + P_y y
  $$
